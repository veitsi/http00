...
if (F_LOGGED and User::info("role_id") == 1) {
    ...
    if ($data["id"] != 0) {
        $nr = DB::con()->query("SELECT COUNT(*) FROM `".F_DB_PREFIX."looks_categories` WHERE `id`=".DB::quote($data["id"])) or die(Fianta::err(__FILE__, __LINE__));
    ...
    if ($data["id"] != 0) {
        DB::con()->query("UPDATE `".F_DB_PREFIX."looks_categories` SET ".$qb->update_string." WHERE `id`=".DB::quote($data["id"])) or die(Fianta::err(__FILE__, __LINE__));
        $result = new AjaxResponse("success", "Успешно сохранено");
        exit($result->json());
    } else {
        DB::con()->query("INSERT INTO `".F_DB_PREFIX."looks_categories` (".$qb->insert_fields.") VALUES (".$qb->insert_values.")") or die(Fianta::err(__FILE__, __LINE__));
